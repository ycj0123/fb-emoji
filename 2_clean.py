...
input_file = 'original_corpus/fb7000.csv'
...
new_data = data.copy()
for i in data.index:
    n_reactions = data['like'][i] + data['love'][i] + data['care'][i] + data['haha'][i] + data['wow'][i] + data['sad'][i] + data['angry'][i]
    if len(data['text'][i]) < 4 or n_reactions < 50:
        # print(data['text'][i])
